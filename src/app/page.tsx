    ...
    <main style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '15px', border: '1px solid #ddd', marginBottom: '30px' }}>
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
          <button onClick={() => setInputMode('書類')} style={{ ...buttonStyle, backgroundColor: inputMode === '書類' ? '#2383e2' : '#f1f5f9', color: inputMode === '書類' ? 'white' : '#64748b', flex: 1 }}>📄 書書類モード</button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div>
            ...
            <input placeholder="URL (D&Dで自動入力)" value={url} onChange={e => setUrl(e.target.value)} style={{ ...inputStyle, marginBottom: '10px', fontSize: '11px' }} />
            
            {/* 【修正】確実なD&Dエリア */}
            {inputMode === '書類' && (
              <div 
                onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }} // ここが重要
              ...
              >
                {uploading ? '⬆️ アップロード中...' : url ? '✅ ファイル準備OK' : '📂 ここにPDFをドロップ'}
              </div>
            )}
